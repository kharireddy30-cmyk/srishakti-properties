...
with tab2:
    ...
    with st.form("sell_form", clear_on_submit=True):
        cat = st.selectbox("Category", list(property_options.keys()))
        ptype = st.selectbox("Type", property_options[cat])
        ...
        if st.form_submit_button("Submit Property"):
            sheet.append_row([cat, ptype, pname, f"{mval} {munit}", price, facing, addr, cp, role, phone, mlink, maplink, status])
